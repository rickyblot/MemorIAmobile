...
import { Share2, Users, Lock, Send, FolderHeart, Download } from 'lucide-react';
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div className="order-2 lg:order-1 relative animate-scale-in opacity-0" style={{ animationDelay: '0.2s' }}>
                ...
                <img 
                  src="https://images.unsplash.com/photo-1643889017792-e606cdc3e7f5?auto=format&fit=crop&q=80&w=800" 
              ...
              <div className="order-1 lg:order-2 animate-slide-up">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-secondary/10 border border-secondary/20 text-secondary font-bold text-sm mb-6 uppercase tracking-wider font-sans">
                  <Share2 className="w-4 h-4" /> {t(`featurePages.${pageId}.heroTag`)}
                </div>
        ...
        <section className="py-24 bg-card border-y border-border">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {[
                { icon: FolderHeart, color: 'text-primary' },
                { icon: Lock, color: 'text-secondary' },
                { icon: Send, color: 'text-primary' },
                { icon: Users, color: 'text-secondary' },
                { icon: Share2, color: 'text-primary' },
                { icon: Download, color: 'text-secondary' }
              ].map((item, index) => (
                <div key={index} className="bg-background rounded-3xl p-8 border border-border shadow-sm flex items-start gap-5 hover:shadow-xl hover:border-secondary/50 hover:-translate-y-1 transition-all">
                  <div className={`w-14 h-14 rounded-2xl bg-card border border-border flex items-center justify-center shrink-0 ${item.color}`}>
                    <item.icon className="w-7 h-7" />
                  </div>
                  <div>
                    <h3 className="font-bold text-xl text-foreground mb-2 font-sans">{t(`featurePages.${pageId}.features.items.${index}.title`)}</h3>
